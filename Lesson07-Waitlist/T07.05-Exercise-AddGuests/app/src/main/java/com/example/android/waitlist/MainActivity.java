...
public class MainActivity extends AppCompatActivity {
    ...
    public void addToWaitlist(View view) {
        ...
        // Complete (9) First thing, check if any of the EditTexts are empty, return if so
         if(mNewGuestNameEditText.getText().toString().equals(null))
             if(mNewPartySizeEditText.getText().toString().equals(null)){
             return;
             }
        ...
        // COMLETE (14) call addNewGuest with the guest name and party size
            addGuest(mNewGuestNameEditText.getText().toString(),partySize);
        // COMLETE (19) call mAdapter.swapCursor to update the cursor by passing in getAllGuests()
            mAdapter.swapCursor(getAllGuests());
        // COMLETE (20) To make the UI look nice, call .getText().clear() on both EditTexts, also call clearFocus() on mNewPartySizeEditText
